...
export function CommunityCards({
  ...
  return (
    <div
      ref={containerRef}
      className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 mt-10 md:mt-20 z-30"
    >
      <motion.div
      ...
      >
        ...
        {Array.from({ length: 5 - displayCards.length }).map((_, index) => (
          <div
            ...
            className="w-10 h-14 md:w-14 md:h-20 border border-dashed md:border-2 border-emerald-600 rounded-lg opacity-30"
          />
        ))}
      </motion.div>
    </div>
