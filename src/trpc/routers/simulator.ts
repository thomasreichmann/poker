...
export const simulatorRouter = createTRPCRouter({
	enable: devOnlyProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			// Enqueue first job soon to kick off processing
			await enqueueSimulatorJob({ gameId: input.tableId, runAtMs: 50 });
			// Kick the worker once to start progress
			await processDueSimulatorJobs(1);
			return { success: true };
		}),
	...
	updateConfig: devOnlyProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			await db
				...
				.where(eq(games.id, input.tableId));
			// Schedule an immediate job in case we changed delays/strategies
			await enqueueSimulatorJob({ gameId: input.tableId, runAtMs: 50 });
			await processDueSimulatorJobs(1);
			return { success: true };
		}),
	...
	resume: devOnlyProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			await db
				...
				.set({ simulatorConfig: next as unknown as object })
				.where(eq(games.id, input.tableId));
			// Schedule a job to resume immediately
			await enqueueSimulatorJob({ gameId: input.tableId, runAtMs: 50 });
			await processDueSimulatorJobs(1);
			return { success: true };
		}),
