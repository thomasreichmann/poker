...
describe("Pure Poker Engine", () => {
  ...
  describe("Action Validation", () => {
    ...
    test("rejects action from wrong player", () => {
      const otherPlayer = gameState.players.find(p => p.id !== gameState.currentPlayerTurn);
  ...
  describe("Action Processing", () => {
    ...
    test("processes bet correctly", () => {
      ...
      expect(result.newGameState.currentHighestBet).toBe(initialCurrentBet + 50); // Player's total bet
      
      const updatedPlayer = result.newGameState.players.find(p => p.id === currentPlayer.id)!;
    ...
    test("processes fold correctly", () => {
      ...
      const foldedPlayer = result.newGameState.players.find(p => p.id === gameState.currentPlayerTurn!)!;
  ...
  describe("Winner Determination", () => {
    test("finds single winner", () => {
      ...
      const player2: Player = {
        ...
        hasWon: false,
        showCards: false,
        holeCards: [{ rank: "2" as const, suit: "hearts" as const }, { rank: "3" as const, suit: "clubs" as const }],
      };
  ...
  describe("Edge Cases", () => {
    ...
    test("handles tie situations", () => {
      ...
      const player2: Player = {
        ...
        hasWon: false,
        showCards: false,
        holeCards: [{ rank: "A" as const, suit: "diamonds" as const }, { rank: "K" as const, suit: "clubs" as const }],
      };
  ...
  describe("Full Game Integration", () => {
    test("4-player full game progresses through all rounds to showdown", () => {
      ...
      const player1: Player = { id: "player1", seat: 0, stack: 1000, currentBet: 0, hasFolded: false, isButton: false, hasWon: false, showCards: false, holeCards: [] };
      const player2: Player = { id: "player2", seat: 1, stack: 1000, currentBet: 0, hasFolded: false, isButton: false, hasWon: false, showCards: false, holeCards: [] };
      const player3: Player = { id: "player3", seat: 2, stack: 1000, currentBet: 0, hasFolded: false, isButton: false, hasWon: false, showCards: false, holeCards: [] };
      const player4: Player = { id: "player4", seat: 3, stack: 1000, currentBet: 0, hasFolded: false, isButton: false, hasWon: false, showCards: false, holeCards: [] };
      ...
      const smallBlindPlayer = gameState.players.find(p => p.currentBet === 25);
      ...
      const totalStacksAfter = gameState.players.reduce((sum, p) => sum + p.stack, 0);
      ...
      const hasChangedStacks = gameState.players.some(p => p.stack !== initialStack);
