...
/**
 * Deprecated: In-memory timers are not serverless-safe. Kept for local dev only.
 */
export class BotManager {
	...
	async start() {
		if (process.env.NEXT_RUNTIME === "edge") return; // disabled on edge
		this.active = true;
		await this.tick();
	}
	...
	async tick() {
		...
		const [game] = await db
			.select()
			.from(games)
			.where(eq(games.id, this.tableId))
			.limit(1);
		if (!game) return this.scheduleNext(500);
		const config = (game.simulatorConfig as unknown as SimulatorConfig) || {
			enabled: false,
		};
		...
		const [pl] = await db
			...
			.from(players)
			.where(
				and(eq(players.gameId, this.tableId), eq(players.id, currentPlayerTurn))
			)
			.limit(1);
		if (!pl) return this.scheduleNext(200);

		const strategyCfg: StrategyConfig | undefined =
			config.perSeatStrategy?.[currentPlayerTurn] || config.defaultStrategy;
		if (!strategyCfg) return this.scheduleNext(200);
		// If this player is marked as human/manual, skip bot action and poll
		if (strategyCfg.id == "human") {
			console.log("Skipping bot action for human player", currentPlayerTurn);
			return this.scheduleNext(200);
		}
		...
		const jitter =
			delays.minMs +
			Math.floor(this.rng() * Math.max(0, delays.maxMs - delays.minMs + 1));
		const waitMs = Math.max(
			0,
			Math.floor(jitter / Math.max(0.1, delays.speedMultiplier ?? 1))
		);
